...
class HypertensionDF:
    ...
    def height_zscore_and_percentile(self, age, sex, height_cm):
        if age < 0 or age > 17:
            return None, None
        filtered_table = self.bp_tables[(self.bp_tables['sex'] == sex) & (self.bp_tables['age'] == age)]
        ...
        z_scores_for_percentiles = [norm.ppf(p / 100.0) for p in height_percentiles_standard]

        z_interpolated = np.interp(height_cm, height_values_unique, z_scores_for_percentiles)
    ...
    def get_hypertension_status(self, age, sex, height_cm, systolic, diastolic):
        ...
        if 1 <= ag < 13:
            ...
            sp_tile = stab.iloc[0] if ht < stab['h.cm'].iloc[0] else (stab.iloc[-1] if ht >= stab['h.cm'].iloc[-1] else stab[stab['h.cm'] > ht].iloc[0] if len(stab[stab['h.cm'] > ht]) > 0 else None)
            sp_percentile = self.calculate_percentile_using_zscore(sbp, sp_tile)
            dtab = filtered_table[filtered_table['bp'] == 'dbp']
            dp_tile = dtab.iloc[0] if ht < dtab['h.cm'].iloc[0] else (dtab.iloc[-1] if ht >= dtab['h.cm'].iloc[-1] else dtab[dtab['h.cm'] > ht].iloc[0])
        ...
        height_z_score, height_percentile = self.height_zscore_and_percentile(age, sex, height_cm)
