...
def app():
    ...
    with col1:
        with st.form(key="subject_form", clear_on_submit=False):
            ...
            height = st.number_input("Height", min_value=0.0, max_value=220.0, step=0.1, format="%.1f")
            st.divider()
    ...
    with col3:
        with stylable_container(
            ...
            tagger_component(
                "**BP Percentiles:**",
                [f'Systolic: **{int(status.get("SPpercentile"))}**', f'Diastolic: **{int(status.get("DPpercentile"))}**'],        color_name=["blue", "orange"],
            )
            st.write(" ")
            st.write(" ")
        # with col2:
            tagger_component(
                "**Height information**:",
                [f'Z score: **{status.get("HeightZScore", 0)}**', f'Percentile: **{status.get("HeightPercentile", 0)}**'],
            ...
            tagger_component(
                "**Categories & Stages:**",
                [f'Systolic: **{status.get("SPhtn", "-")}**', f'Diastolic: **{status.get("DPhtn", "-")}**'],
                color_name=["blue", "orange"],
            )
            st.write(" ")
            st.write(" ")
    ...
    def plot_dbp_chart_echarts(dbp_data, sex, subject_age=None, subject_diastolic=None):
        ...
        echart = {
            ...
            'legend': {
                'bottom': 0,
                'data': line_labels + (['Subject'] if subject_age and subject_diastolic else [])
            },
            'xAxis': {
                'type': 'category',
                'min': 0,
    ...
    with col2:
        with stylable_container(
            ...
            # c1, c2 = st.columns(2)
                st_echarts(plot_sbp_chart_echarts(
                            sbp_data, 
                            sex, subject_status["age.y"], 
                            subject_status["systolic"]),
                        height=250,)

                st_echarts(plot_dbp_chart_echarts(
                            dbp_data, 
                            sex, subject_status["age.y"], 
                            subject_status["diastolic"]),
                        height=250,)
